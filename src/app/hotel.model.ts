...
export class Hotel {
  ...
  public checkIn(
    ...
    huespedes: Array<Persona>
  ): number {
    var habitacion: number = 0;
    for (var i = 0; i < this.lista.length; i++) {
      if (
        this.lista[i].getCategoria() === tipoHabitacion &&
  ...
  public registrarHuesped(
    ...
    if (habitacion.getListaHuespedes().length != 0) {
      if (habitacion.getListaHuespedes().length < habitacion.getCapacidad()) {
        ...
        resultado =
          'No se puede registrar, supera capacidad máxima de huespédes: <' +
          habitacion.getCapacidad() +
          '>';
    } else resultado = 'Se debe hacer checkIn para registrar el Responsable';
  ...
  public darSalida(id: number): void {
    ...
    this.lista[id - 1] = new Habitacion(
      id,
      habitacion.getCategoria(),
      habitacion.getCapacidad(),
      habitacion.getResponsable()
    );
  }
  ...
  public existResponsable(cedula: number): boolean {
    ...
    for (var i = 0; i < this.lista.length; i++) {
      if (
        !this.lista[i].isDisponible() &&
        this.lista[i].getResponsable().getCedula() == cedula
      ) {
        resp = true;
        i = this.lista.length;
      }
